...
### 4.6 拆开看：最大的一笔是我们自己的缓存在每次命中时深拷贝

§4.5 里我曾推断「剩下的 20~28 ms 全在暴露面判据和遮挡射线上」。**那是猜的，而且是错的。**
把成本拆开量之后（harness 同一个文件，另加两段微基准 + 一次性的命中计数脚手架）：

**单位成本**（release，min-of-5）：

| 操作 | 空气 | 石头 |
|---|---|---|
| `block_snapshot` 建 DTO | 83 ns | 312 ns |
| `BlockReadResult::clone` | 50 ns | **125 ns** |

**每次投影的缓存命中/未命中次数**（确定性，不受机器负载影响）：

| 场景 | 命中 | 未命中 | 倍数 |
|---|---|---|---|
| 全空气 | **17** | 20 211 | 0.0x |
| 露天平地 | 125 767 | 20 956 | 6.0x |
| 洞穴气泡 | 117 336 | 21 744 | 5.4x |
| 30% 噪声 | **179 724** | 20 728 | 8.7x |
| 全石头 | 116 629 | 21 769 | 5.4x |

`viewport.rs:352` 是 `block_cache.get(&key).cloned()`——**每次命中都把整个
`BlockReadResult` 深拷一份**，而它带着 `Vec<[f64;6]>` 和 `BTreeMap`。
暴露面判据每个候选要探 6 个邻居，邻居基本都是已读过的候选，于是命中是未命中的 5~9 倍。

**全石头 36.9 ms 的分解**（算术与实测吻合到 ~2 ms）：

| 成分 | 估算 | 占比 |
|---|---|---|
| 缓存命中的深拷贝 | 116 629 × 125 ns = **14.6 ms** | **40%** |
| 建 DTO（未命中） | 21 769 × 312 ns = 6.8 ms | 18% |
| 扫描地板 | ≈ 7.5 ms（全空气 9.2 减去它自己的 DTO 1.7） | 20% |
| 其余（暴露面数学 + 12.6 万次 HashMap 哈希） | ≈ 8 ms | 22% |

对照组是全空气那行：没有候选就没有邻居探测，只有 17 次命中，
所以它的 9.2 ms 正是纯扫描 + 建 DTO 的地板。

**这个缓存不是没用**（省一次 312 ns 的 DTO，花一次 125 ns 的拷贝），
**但它本可以几乎免费**。
...
| # | 做法 | 攻击哪一块 | 量级 | 来源 | 风险 |
|---|---|---|---|---|---|
| 1 | 缓存改存 `Rc<BlockReadResult>`，命中只加引用计数 | 深拷贝 14.6 ms | **最大** | 不是原版，是我们自己的漏 | 低：18 处签名全在 `viewport.rs` 内，公开入口不动，不改行为 |
| 2 | 廉价探针代替完整 DTO：2 万次里只有 ≤256 次真要完整 DTO，其余只需 `transparent_hint` + `bounding_box` | 建 DTO 6.8 ms | 中 | 不是原版 | 中：要拆 `BlockReadResult` 的形状 |
| 3 | section 连通图（原版 cave culling）：预计算每个 section 六个面之间的连通性，从相机 section 做 BFS | 扫描地板 7.5 ms；**地下/洞穴场景是数量级的** | 场景相关 | **原版最值钱的一条** | 高：只能当**预筛**——它保守（"可能可见"），我们不能有假阳性 |
| 4 | 网格缓存（原版按 section 缓存暴露面，变更时失效） | 暴露面数学 | **最小** | 原版 | 中 |

**为什么第 4 条最不值钱**——直觉上"缓存暴露面"该是大头，但计数说不是：
邻居探测**几乎全是缓存命中**（五种地形的未命中数几乎一样，20.2k~21.8k），
所以第 1、2 条做完之后，暴露面那部分本来就已经很便宜了。
而且我们的暴露面判据是**朝向观察者**的（`viewport.rs:60-62`），视点相关；
原版的网格是视点无关的。能缓存的只有"哪些面存在"这半边。

**不可迁移的一条**：原版的精确遮挡是深度缓冲白给的，GPU 干的。
我们没有等价物，射线躲不掉——只能靠第 3 条把它减少，减不到零。

### 4.8 做了第 1、2 条：快 5~8 倍，「1 tick」重新可谈

维护者裁定「可以优化，不改公开契约不改行为的顾虑不必有，我们没有负担」，
于是把第 1、2 条合并做掉了（原版的第 3、4 条**没做**）。

**做法**：`ProtocolBlockSnapshot` 除 `position` 外每个字段都是 `state_id` 的纯函数，
而扫描热路径（`is_visible_block` / `is_occluding_block`）只读其中两位：
是不是空气、透不透光。于是：

- 新增 `BlockProbe`（`Copy`，两位事实）与按 `state_id` 索引的静态表
  `snapshot::block_probe`——探测一格是一次数组下标，零分配；
- 投影改用 `WorldReader { probe, full }` 两层读取：**扫描与射线只走探针**，
  **只有要交给模型的方块才建完整 DTO**；
- 块缓存改存探针（`Copy`），命中不再深拷；
- `BlockInfo` 推迟到**排序截断之后**才建（排序只用距离与坐标，次序不变）；
- 内部射线 `first_hit` 只返回命中坐标——三个调用方里两个本来就只关心命中与否。

**行为等价性**：新增测试 `block_probe_agrees_with_the_full_dto_for_every_state`
对**每一个** `BlockState` 断言探针与走完整 DTO 的结论一致；
后端 207 个测试全过，全工作区除两个**先前就在失败**的
`information_runtime` panic 测试（本分支删空 `catch_unwind` 的后果，与本次无关）外全过。

**实测**（同机、机器干净、参数与改前完全相同）：

| 场景 | 改前中位 | 改后中位 | 倍数 | 改后占一刻 | 完整读/可见块 |
|---|---|---|---|---|---|
| 全空气 | 9.19 ms | **1.78 ms** | 5.2× | 4% | 0 / 0 |
| 露天平地 | 29.29 ms | **5.00 ms** | 5.9× | 10% | 257 / 256 |
| 洞穴气泡 | 36.36 ms | **4.67 ms** | 7.8× | 9% | 62 / 62 |
| 30% 噪声 | 33.27 ms | **7.33 ms** | 4.5× | 15% | 39 / 37 |
| 全石头 | 36.94 ms | **4.86 ms** | 7.6× | 10% | 2 / 0 |

最慢单样本从 60.5 ms 降到 12.96 ms。「完整读」与「可见块」现在同量级，
这正是这次改动要达到的形状——中间一度是 12 597 / 37，
那是内部射线仍在为每次命中建 `BlockInfo`，也一并修了。

**对 §7 第 1 条的影响**：单次 `view` 现在占一刻的 4~15%。
「工具数组在 1 tick 内处理完」不再被 `view` 一个人否掉——
但 `MAX_TOOL_CALLS_PER_RESPONSE = 8`，8 次 `view` 仍会超。
⚠ 目标怎么写还是维护者裁定，只是现在两个选项都成立了。

**没做的两条仍然摆着**：原版的 section 连通图（攻击扫描地板，
地下场景可能再降一个量级）和网格缓存。现在没有理由做——
剩下的成本里最大的一块是扫描本身（全空气 1.78 ms 就是它的地板）。
