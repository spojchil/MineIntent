...
**紧迫性有数了（2026-08-19 实盘）**：这条裁定还没实现，而它治的正是当前上下文
增长的正主。第二跑 3 分钟：请求 #53 一次涌进 **85 条转录、+32,215 token**（走路时
帧每 250ms 一发，模型忙着就全排在信箱里一次并进来）；3 分钟 **164 帧**、累计
**4,876 条方块差异**；输入峰值 **111,617**——按这个斜率 30 分钟撞上 1M 窗口。
模型原话：「我看到了一路走过的大量地形信息」。详见
[validation.md](./guides/validation.md) 的 08-19 两跑。
...
- **触发不变，仍然只看方块差异**。处境里的位置几乎每帧都变，让它自己触发等于站着不动
  ...
  > ⚠ 2026-08-21 更正：这句只在写下的那一天为真。`fe48330`（2026-08-20）把触发
  > 从「方块差异非空」改成**只有 job 进展**——因为方块差异随后整个退出了会话区
  > （裁定五），再拿它当触发源就没有意义了。处境仍然搭车，这一半没变。
  > 代码里同一句谎由 `5593799` 修掉，文档这句到今天才修。
...
**先查文档。**<https://api-docs.deepseek.com/quick_start/pricing> 直接列了每个模型的
Context Length 与 Max Output。API 查不到——DeepSeek 的 `/models` 与 `/models/{id}` 都只
返回 `id` / `object` / `owned_by`。文档没写时才用报错法（服务商越界时会说出来，两步都
不计费）：`max_tokens: 99999999` 问输出上限，超量 messages 问上下文窗口。

2026-08-19 查证的三件事：

| | |
| --- | --- |
| 真正在跑的模型 | `MODEL_NAME=deepseek-chat` **实际解析到 `deepseek-v4-flash`**（回包的 `model` 字段自己说的） |
| 窗口 / 输出上限 | **1 048 576**（2^20）／393 216 |
| 输入价 | 命中 **$0.007/M**，未命中 **$0.22/M**（峰时翻倍） |

### 命中与未命中差 31 倍——这才是前缀那笔账的分量

`docs/guides/validation.md` 记的是 token 数。换成钱看：

| | |
| --- | --- |
| 13 次轮首未命中 367,984 token | 实付 **$0.081**（峰时 $0.16） |
| 同样这些 token 若全命中 | **$0.0026** |

**同一份上下文，位置放错，30 分钟一跑贵 31 倍。**处境移出前缀省下的不是 75% 的 token
数，是 75% 乘以这个倍数。

### 两条线必须联动，不能各配各的

先前 token 线 996 147、字节线 1 MiB（≈29 万 token）——**字节线永远先撞上，token 线等于
不存在**。现在字节线从 token 线算出来（×4 字节/token，实测 3.6，故意往高取），
`MINEINTENT_COMPACT_ABOVE_BYTES` 仍可覆盖但默认不再需要动。字节线只在第一次请求之前
有效：没有 usage 可读时的兜底。
...
### 裁定五（2026-08-20）：方块信息根本不进会话区

裁定二本想「屏蔽会话历史里的方块信息，改从 `BlockMemory` 投影呈现」，但**投影**那半
在数学上不成立。三条路都死：

| 做法 | 死在哪 |
| --- | --- |
| 逐格差异（旧） | 小，但**永久累积**——3 分钟 111k，十几分钟吃掉半个窗口 |
| 每轮从记忆全量投影 | 不累积，但一轮就 4.8 万格 |
| 压缩时连记忆一起清 | 下一次 diff 全部重新 `Appeared`，7,040 格一次爆发，比省下的还多 |

按实测 **10.5 token/格**（3.6 字节/token × 约 38 字节/行；`diff_mix_probe` 测出
100% 是 `Appeared`、行/条 = 1.00，没有 2 倍虚高），半个 1M 窗口只装得下约 **4.8 万格**，
而站着转两分钟就攒 **7,040 格**。**逐格进上下文是量级不对，不是优化不够。**

**所以裁定改成：一格都不推。**

- 差异照常算、照常推进 `BlockMemory`——那是**给机器用的**（寻路读它，`blocks` 工具
  查它），不进上下文；
- 轮末帧里不再有方块，只剩处境行与 job 进展；
- 模型要方块就自己 `scan`（睁眼，把看见的吸进记忆），再用 `blocks` 查那本记忆。

### 类比：改一个大仓库

维护者点破的：人改大仓库从不加载全部——`grep` 到位置，再读那几行；记的是结构不是
原文；**压缩丢掉原文不丢能力，因为磁盘还在**。

`BlockMemory` 就是那块磁盘。它**不该被压缩碰**，会话区里本来就不该有原文。冷启动因此
也不是问题：「什么都不知道」和「什么都没推」是同一件事，`scan` 一下就有了。

### 但查询不是 grep

grep 按模式列举、让调用者自己解释；模型问的是「附近有什么树」，不是
`oak_log|spruce_log|birch_log`。所以：

- **入口按用途收词**（输入侧翻译，一张写死的别名表）；
- **出口一律方块事实**——坐标、标签、方位距离。

⚠ **机器不产出解释。**「上方 5 格是天空」「水面宽约 12 格」「铁矿一般在更深处」这类
都是替模型下结论，与读法 B 被否是同一个理由。「这是悬崖」「那片林子」由模型自己看
出来。这也让「在场感需要一段纯函数地貌描述」成了伪问题——需要的只是足够的方块事实。

### 后续（未做）

- **增量怎么处理**：现在只喂记忆，给模型的形态未定；
- **监视/订阅**：记忆成为磁盘之后，「盯着这个熔炉」才有意义——推与拉的死结解在这里，
  模型声明关心什么，只推它声明过的。这也正是「模型自己决定轮末帧内容」那条的归宿。
...
### 屏用法裁定（2026-08-17）：用法全文不随开屏投递

> 本条原题也叫「裁定三」，与 §2 的裁定三（处境移出前缀）撞号而主题无关。
> 2026-08-21 去掉编号——全仓引用「裁定三」的四处指的都是 §2 那条。
...
2. ~~**触发条件**。现状 `compact_above_bytes: 256KB` 是绝对字节，与模型窗口脱钩，
   换模型即失准。~~ **已由裁定四解决**（2026-08-19）：token 线设在供应商窗口的
   95%，字节线跟着 token 线算而不单独配。留在清单里是因为「在系统性提前」
   （Gemini CLI 95% → 70% → 0.5）那一半仍未裁——不过压缩眼下是空实现，
   越线只是观察点，这一半不急。
...
| `crates/companion/src/main.rs` | `session_config()`（阈值；裁定四起按窗口 95% 算）、帧的观察端与 `WhenIdle` 投递（`94b6616` 起不再是 `Passive`） |
