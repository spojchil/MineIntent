...
| `inventory open` 全清单 + 用法 | **~1 100** | 其中 **~700 字节是用法全文** |
| 容器开屏（工作台）清单 + 用法 | **~1 400** | 其中 **~900 字节是用法全文** |

⚠ 已看出的浪费：**开屏用法全文每次开屏都重发一遍**。它是纯静态文本，本该在工具表里
（免压缩、吃缓存），现在在回执里（会话区、每次重付）。开三次工作台就进上下文三份。
...
8. **唤醒投递不可重导出**。「alice 说了什么」若不入摘要就真没了，它没有第二份来源，
   与方块不同。
9. **开屏用法全文该挪进工具表**（§3 的浪费项）。与压缩正交，可独立做。
10. **压缩的可观察性**。现在压缩成功或放弃全程静默，四条 `return unchanged()`
