...
> **历史文档。** 本文梳理的是旧栈的欠账与复杂处，那些代码已删除。
> 其中 §8.2「拆分只减了文件体积，没有建立模块边界」是当前仍在遵守的教训——
> `crates/world/src/machine/` 的拆分就是按它做的。
...
>
> **配套文档**：本文是**静态**梳理。之后在 `experiment/no-panic-live` 分支上做了
> 四轮真实盘（真 Paper 26.1.2 + 真模型），观测记录见
> [删掉全部 panic 捕获之后的实盘观测](./no-panic-live-run.md)。
> 两份文档相互印证与相互修正的地方，在 §10 的「后续补充」里逐条标出。
...
> **实盘证据**（[实盘观测](./no-panic-live-run.md) §3.2）：用 RCON 打死同伴那一轮，
> journal 全部 20 条里有 **15 条 `participant_events_omitted` + 4 条 `overflow`**，
> 只有 1 条是真事实（死亡的 `lifecycle`）。
> 死亡引发的区块重载风暴在**一秒内**产出 13 条遗漏标记。
> 「journal 里 19/20 条是『我丢了东西』」不是推演，是实测。
...
1. **本文成文时没有实盘运行。** 全部结论来自静态阅读、`rg` 计数、`cargo check/test`
   与一次模块摘除编译实验。

   > **后续补充**：之后在 `experiment/no-panic-live` 分支上做了四轮实盘
   > （真 Paper 26.1.2 + 真模型），结果见[实盘观测](./no-panic-live-run.md)。
   > 它证实了 §3.4（死亡瞬间一秒内 13 条遗漏标记）、
   > 部分否证了 §4.1（那些 `panic!` 一次也没被触发，见下条），
   > 并暴露了本文完全没有覆盖的一类问题：**停机死锁**。

2. **§4.1 的 panic 风险没有触发验证。** 我确认了「注释所说的接管者不在生产路径上」，
   但没有构造 backend 返回 `Err` 的场景去看它是否真的 panic。

   > **后续补充**：实盘四轮（含硬断线与游戏内死亡）里，
   > `information_adapters.rs` 那 8 处 `panic!` **一次也没有触发**——
   > 触发条件 `backend.snapshot()` 返回 `Err` 即使在硬断线时也没有出现
   > （观察面按设计在断线/死亡时仍可读）。
   > 「没有接管者」这个结论仍然成立，但「会不会被打到」现在有了负面证据。
