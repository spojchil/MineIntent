...
> **历史文档。** 本文描述 TypeScript 原型栈（`src/`、`agent-service/`、`mcserver/`）。
> 那套代码在本分支已删除，**仍完整保留在 `main`**——两条线从未合并。
> Rust 单进程实现见 [`docs/architecture.md`](../architecture.md)。
...
Node 入口是 [`src/main.ts`](https://github.com/spojchil/MineIntent/blob/main/src/main.ts)，对象装配位于
[`MineIntentApp.start()`](https://github.com/spojchil/MineIntent/blob/main/src/app/mineintent-app.ts)。它创建 Minecraft Backend、事件日志、记忆、
Agent Service 客户端、工具回调和只读调试服务。

Python 入口是 [`agent-service/server.py:main()`](https://github.com/spojchil/MineIntent/blob/main/agent-service/server.py)。它只监听 `127.0.0.1`；Node 不负责
...
[`AgentServiceModelProvider`](https://github.com/spojchil/MineIntent/blob/main/src/models/agent-service-client.ts)。

Python 收到模型工具调用后，通过 Node 临时创建的 `POST /v1/tool` 回调执行。回调只绑定 loopback，使用独立随机令牌，
并验证 run、tool call、工具名和参数。产生方是 [`ToolBridgeServer`](https://github.com/spojchil/MineIntent/blob/main/src/models/tool-bridge.ts)及
[模型契约](https://github.com/spojchil/MineIntent/blob/main/src/models/contracts.ts)。
...
进行中对话机制，但本次运行时装配不传该值。产生方是 [`ParticipantRuntime`](https://github.com/spojchil/MineIntent/blob/main/src/participant/runtime.ts)和
[`interpretPlayerChat()`](https://github.com/spojchil/MineIntent/blob/main/src/speech/chat-input.ts)。
...
协议类型见 [`src/models/contracts.ts`](https://github.com/spojchil/MineIntent/blob/main/src/models/contracts.ts)，frame 组合见
[`composeAgentContext()`](https://github.com/spojchil/MineIntent/blob/main/src/information/context-composer.ts)。完整视野不在 opening frame 中；opening 只含姿态和
坐标图例。

Python 将通用系统提示词和记忆组成 system message，再追加 opening frame。之后只追加 assistant tool calls 和
对应的 tool results，不重写旧消息。实现见 [`prompt.py`](https://github.com/spojchil/MineIntent/blob/main/agent-service/prompt.py)与
[`run_tool_loop()`](https://github.com/spojchil/MineIntent/blob/main/agent-service/server.py)。
...
[`MinecraftBackend`](https://github.com/spojchil/MineIntent/blob/main/src/minecraft/minecraft-backend.ts)使用 Mineflayer 连接固定版本 `1.21.1`，维护连接状态、快照、
...
装配发生在 [`buildInformationRuntime()`](https://github.com/spojchil/MineIntent/blob/main/src/participant/runtime.ts)。Information Runtime 还实现 catalog、help、selector、
...
[`ViewportInformationProvider`](https://github.com/spojchil/MineIntent/blob/main/src/information/providers/viewport-provider.ts)。
...
能力在 [`src/participant/capabilities/`](https://github.com/spojchil/MineIntent/blob/main/src/participant/capabilities/)注册；同一 registry 同时产生模型 schema 和执行分派。
...
[`FileMemoryStore`](https://github.com/spojchil/MineIntent/blob/main/src/memory/memory-store.ts)和 [`remember`](https://github.com/spojchil/MineIntent/blob/main/src/participant/capabilities/remember.ts)。
...
[`src/telemetry/`](https://github.com/spojchil/MineIntent/blob/main/src/telemetry/)。
...
仓库同时存在两套实现。上面的 TypeScript 栈仍可运行、仍被验证指南覆盖；
`crates/` 是全 Rust 单进程实现，结构见 [Rust 实现结构](../architecture.md)，
运行方式见 [workspace 指南](../guides/rust-workspace.md)。

```text
Minecraft Java 26.1.2（Paper，协议 775）
        ↕ Azalea（自有 fork，rev 钉死）
Rust 单进程 —— companion 是唯一组合根与唯一可执行
```

与 TS 栈的**结构性差异**（均已有裁定，不是实现偏差）：

- Python agent-service 与 loopback HTTP 工具桥不再存在，模型—工具循环内联为
  进程内的 `agent` 内核；
- 目标服务端版本从 1.21.1 升到 26.1.2，协议执行层从 Mineflayer 换成自有 fork 的 Azalea；
- 世界信息**改走状态**：世界长什么样经视口拉取到达，事件通道只留「状态里不留痕」
  的那一类（聊天、受伤、任务终局）；
- 上下文有压缩策略与跨轮连续性（TS 栈没有）。

TS 栈在移植期的地位是**行为 oracle**：137 个行为测试是可执行的行为规范，
逐条对应关系见[历史索引](./index.md)中的迁移证据。

⚠ 早期的 Rust 移植（`crates/{app,backend,middle,contracts,toolloop}`）已在
`refactor/single-stack` 删除，其说明、审计与实盘证据见
[`docs/history/`](./)。
