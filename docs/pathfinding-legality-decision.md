...
## 4.5 实测（2026-08-19，`legal_path_probe`）

普通世界、release、`allow_mining(false)`、`BlockPosGoal`。

**自检先过**：记忆为空时合法这边**找不到路**（全量找到 7 节点）。什么都没看过就
哪儿也去不了——实现确实生效。

环视一圈（八向 × 三档俯仰 = 24 次 `scan_changes`）攒到 **1382 格**之后：

| 目标距离 | 全量世界 | 合法地图 |
| --- | --- | --- |
| 4 格 | 找到 4 节点 完整 · 0.4ms | **找到 4 节点 完整** · 5.5ms |
| 8 格 | 找到 7 节点 部分 | **找到 7 节点 部分** |
| 16 格 | 找到 16 节点 完整 · 0.9ms | **找到 16 节点 完整** · 6.7ms |
| 32 格 | 找到 32 节点 完整 | 找到 **18 节点 部分** |
| 64 格 | 找到 64 节点 完整 | 找到 **18 节点 部分** |

**拐点是 18 节点**：1382 格的记忆支撑约 18 格的自主移动，再远就截在知识边界。

**「部分」不是失败。**32 与 64 格都停在同一个 18，那就是边界本身。azalea 的
partial path 语义是「先走到这儿」——走过去、看到更多、记忆增长、重规划。D\* Lite
那个循环不用我们写，自动成立。

耗时：完整路径 0.4→5.5ms、0.9→6.7ms（慢 6~13 倍，绝对值都在 10ms 内）；部分路径时
合法反而更快（28ms vs 106ms），因为搜索空间小。**维护者裁定这一项不构成问题**——
真要优化，让来源直接吐 bitset、只遍历记忆里那几格即可，现在不动。

⚠ 探针局限：用 `BlockPosGoal` + 固定 y，**目标格自己可能就站不住**——8 格那行连全量
都是「部分」，多半是目标点悬空或在方块里，不是路不通。32/64 两行的结论不受影响
（合法停在 18 而全量走到底，差在知识边界），但 8 格那行不能拿来说事。要更干净的
对比该换 `XZGoal` 或先验证目标可站立。

## 4.6 只换数据，别接管控制（2026-08-19 教训）

这条裁定本来只有**一件事**：把寻路读的世界换成观察过的那一份。我在上面又加了
「多段行走」——`PathEnded` 时自己 `stop_pathfinding()` 再 `begin_goto()`，结果打断了
azalea 自己的续接：

```rust
// execute/mod.rs：recalculate_near_end_of_path
if (executing_path.path.len() == 50 || executing_path.path.len() < 5)
    && !pathfinder.is_calculating
    && executing_path.is_path_partial
{
    goto_events.write(GotoEvent { entity, goal, opts });   // 用同一个 goal 自动重发
}
```

**部分路径快走完它自己会用同一个目标重算。**走过去、记忆长了、下一段更远——
D\* Lite 那个回路 azalea 已经闭合了，不需要我们再写一遍。

代价是实盘一跑作废：模型发完 `go_to` 三分钟收不到任何东西（我的重发是静默的）。

**留下的是只读那半**：读 `ExecutingPath.path.back()`，每程开始报一句这一程走到哪。
读端点不碰控制，既便宜又有用。

### 顺带查清的两件事

**「不可达」azalea 怎么判的**：它保留 7 个候选中途点（`COEFFICIENTS` 七种权衡，
系数大 ≈ 只管靠近、系数小 ≈ 也要省力），`determine_best_path_idx` 取第一个不等于起点
的。七个全等于起点才是 `No best node found`。所以**「走到附近」本来就在算法里**，
那七个候选就是附近的落脚点。

**远距离从来不是限制**（2026-08-19 实测，同一起点）：

| 目标距离 | 全量世界 | 合法地图 |
| --- | --- | --- |
| 16 格 | 11 节点 部分 | 11 节点 部分 |
| 64 格 | 64 节点 部分 | 25 节点 部分 |
| 128 / 512 / 2000 / **8000 格** | 101 节点 部分 | **25 节点 部分** |

128 格以上全量一律 101 节点——那是搜索预算上限，不是距离。合法版稳定 25 节点，
就是它知道的那一圈。**两边到 8000 格都返回部分路径。**

实盘那次 `No best node found` 是**相邻一格**的目标：人在 (-12,64,-45)、目标
(-12,64,-46)，目标格的地面没观察过，而人已经贴着它——任何一步都是走远，七种权衡
下没有一个节点比起点更好。A\* 的结论完全正确，**近处失败比远处彻底**。

真正错的是 `go_to` 用 `BlockPosGoal`（要求精确站上那一格）。模型的意图是「走到树
旁边」，`ReachBlockPosGoal`（4.5 格够得着即可）当场就满足。**未改**。
