...
//! # 什么时候到
//!
//! 眼下走 `WhenIdle`：**只有完成边界合格**——模型不再调工具、这一轮本该收尾的
//! 那一刻才排空。（排空到非空内容时内核并不收尾，而是接着再请求一次模型，所以
//! 帧从来不是「一轮结束之后」才到的。）一轮里工具批之间的那些边界全部跳过。
//!
//! 这件事此前叫「轮末帧」，那个名字把它说歪了：听起来像时机，实际是**跳过了一
//! 轮里所有工具批之间的边界**。实盘量过一轮请求 14 次，第 2 到第 14 次之前一帧
//! 都没有；一场 27 次请求的会话总共只投出去 2 帧。模型的动作按批发生，它的处境
//! 却要等它不再调工具才更新。换投递模式是下一批的事，这一批只搬家。
//!
//! # 为什么只有进展能发车
//!
