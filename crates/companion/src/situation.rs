...
//! # 为什么处境不在前缀里
//!
//! 到 2026-08-18 为止，处境是 `ContextStrategy::base_context` 的第三项，每轮从最新
//! 快照现渲染。内核把 `base_context` 放在每次请求最前面，而前缀缓存按最长公共前缀
//! 命中——于是每轮开头缓存都断在处境那一项，它后面的整条对话全额重算。
//!
//! 实盘 30 分钟、456 次请求的账：
//!
//! | | 未命中 |
//! | --- | --- |
//! | 13 次轮首 | **367,984**（占全跑 75%） |
//! | 443 次轮内 | 120,668（均值 272） |
//!
//! 轮首命中值是 `0 / 256 / 640`——不是没命中，是只命中了人设加半截记忆就断在处境
//! 开头。旧栈 PR #93（2026-07-27）立的规矩说得很清楚：**追加一次是免费的，同一段
//! 东西每轮重渲染是致命的**。Rust 迁移时这条分工丢了，本模块把它装回去。
//!
//! # 只说变了的那几行
//!
