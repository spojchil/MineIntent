...
/// 自称一行：你在这个世界里叫什么。
///
/// 人设是静态配置，不知道运行时用户名；处境若不自称，模型在聊天窗里读到
/// 「MineIntentBot joined the game」与「Alice: MineIntentBot ……」时，
/// **两条都会当成关于第三方的话**——不认识自己的名字，就分不清哪条任务
/// 是派给自己的。
///
...
/// 环境一行：维度、群系，有雨雪雷才提天气。
///
/// **时段不进这一行**：原版客户端的调试屏（26.1.2 的 45 项 `DebugScreenEntries`）
/// 里没有一天内时刻这一项，唯一碰时钟的 `DAY_COUNT` 读的是天数——洞内也不可见，
/// 判据不过。模型主动抬头看天是另一回事，那走 `day_time` 字段。
///
...
/// 直译原则，不装懂。
///
/// 组合根与 `model_surface` 导出**共用这一份**——两边各拼各的曾导致一处改动
/// 只落了一边，「保真导出」当场失真。
