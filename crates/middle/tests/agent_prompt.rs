...
fn initial_messages_keep_stable_system_before_one_appended_frame() {
    ...
    assert_eq!(frame, serde_json::to_value(&context.frame).unwrap());
    assert_eq!(frame["light"], 12);
    assert_eq!(frame["hotbar"]["slots"]["0"], json!(["oak_log", 12]));
    assert_eq!(frame["chat"]["items"][1]["moved"], "events");
    assert_eq!(frame["events"][0]["type"], "player_chat");
    assert_eq!(frame["events"][0]["text"], "帮我看看农田");
    for forbidden in [
        "player",
        "self",
        "inventory",
        ...
        assert!(frame.get(forbidden).is_none(), "{forbidden}");
    }
    assert!(!messages[0]["content"].as_str().unwrap().contains("看看羊"));
}

#[test]
fn v4_opening_replay_requires_an_explicit_legacy_helper() {
    let context = fixtures::agent_context_v4();
    let messages = initial_messages_v4_legacy(&context, &template())
        .expect("legacy v4 opening composes explicitly");
    let frame: Value = serde_json::from_str(messages[1]["content"].as_str().unwrap())
        .expect("legacy opening frame is JSON");
    assert_eq!(frame["player"]["text"], "看看羊");
    assert!(frame["world"].get("timeOfDay").is_none());
}
