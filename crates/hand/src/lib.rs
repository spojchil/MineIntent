    ...
    async fn dispatch(&self, call: ToolCall) -> ToolResult {
        ...
        let outcome = match action {
            ...
            _ => {
                return ToolResult::failure(
                    call_id,
                    "action 必须是 attack/mine/pillar_up/place/use_on/use_item/release/drop/swap_offhand/select_slot 之一；请改写调用",
                )
            }
        };
        match outcome {
        ...
        let mut definition = ToolDefinition::new(
            ...
            json!({
                ...
                "properties": {
                    "action": {
                        "type": "string",
                        "enum": ["attack", "mine", "pillar_up", "place", "use_on", "use_item", "release", "drop", "swap_offhand", "select_slot"],
