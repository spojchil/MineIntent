...
export function interpretPlayerChat(
  ...
  const explicitName = mentionsName(event.payload.plainText, context.participantUsername)
  const ongoing = equalName(context.conversationActiveWith, sender)
  const onlinePlayers = context.onlinePlayerUsernames.filter(name => !equalName(name, context.participantUsername))
  ...
  return {
    ...
    addressing: { addressedToParticipant: addressed, evidence: evidence.length ? evidence : ['not_addressed'] },
