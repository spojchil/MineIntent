 ...
 *
 * It owns exactly three things — resource leases, job lifecycle, and epoch binding — and
 * deliberately knows nothing about goals or targets. "Hold forward until I say stop or 800ms
 * elapse, and tell me when you stop" is in scope; "walk to the sheep" is not. Holding a target is
 * what grew the previous Action Runtime into a planner, and D01 has since ruled that path out, so
 * the boundary is the whole point of rebuilding this smaller.
 */
...
export interface ExecutionRefusal {
  ...
  code: 'resource_busy' | 'unknown_tool' | 'scope_invalid'
  summary: string
}
