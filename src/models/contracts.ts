 ...
 *
 * Frames enter the conversation by being *appended* — as the opening user content or inside a
 * tool result — and are never re-rendered. That distinction is the whole design: under prefix caching a
...
/** One model-visible tool answer and any world observation collected after it was handled. */
export interface ToolExecution {
  result: unknown
  /** Temporal placement only: events in this observation need not have been caused by the tool. */
  observationAfter: AgentFrame | null
}
 ...
 * from the model's tool call through the action to the journal. It is printable ASCII, must be
 * unique within a run, and is claimed before execution to prevent replayed effects. `runId`
 * supplies the authoritative lifecycle and world scope; the transport does not invent another
 * identity for a model response.
 */
export const toolInvocationSchema = z.strictObject({
  runId: z.string().min(1).max(128),
  toolCallId: z.string().regex(/^[\x21-\x7e]{1,128}$/u),
